...
LRESULT CALLBACK WindowProc(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    switch (uMsg) {
        ...
        case WM_PAINT: {
            ...
            // Load the font
            int fontsAdded = AddFontResourceExW(customBanglaFont, FR_PRIVATE, 0);

            // Check if the font was loaded successfully
            if (fontsAdded > 0) {
                wprintf(L"Font loaded successfully! Number of fonts added: %d\n", fontsAdded);
            } else {
                wprintf(L"Failed to load font. Check the file path or permissions.\n");
            }
